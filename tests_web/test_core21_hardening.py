...
def test_web_login_does_not_return_bearer_token_to_browser():
    ...
    assert 'access_token=(None if str(payload.client).lower() == "web" else token)' in APP


def test_correction_review_route_has_no_accidental_extra_parameter():
    assert "def review_correction_request(payload: CorrectionReviewRequest, user: dict = Depends(current_user)):" in APP
