...
from functions import (
    ...
    create_clinic, list_clinics, set_clinic_status, assign_user_to_clinic, list_user_clinics,
    assign_patient_to_clinic, list_patient_clinics,
)
...
class RecordGuardApp(tk.Tk):
    ...
    def _render_organization_hub(self):
        ...
        org=self.current_user.get("organization_id") or "DEFAULT"
        f=self._card(c); tk.Label(f,text=f"Organization: {org}",bg="white",fg=COLORS["ink"],font=("Segoe UI",15,"bold")).pack(anchor="w",padx=18,pady=(16,4)); tk.Label(f,text="Users and patients are scoped to this organization. Clinic assignments add an explicit operational boundary without changing clinical authorization.",bg="white",fg=COLORS["muted"],font=("Segoe UI",9),wraplength=850).pack(anchor="w",padx=18,pady=(0,12))
        actions=tk.Frame(f,bg="white"); actions.pack(fill="x",padx=18,pady=(0,14))
        ttk.Button(actions,text="Manage users",style="Accent.TButton",command=lambda:self._route("_users")).pack(side="left",padx=(0,8))
        ttk.Button(actions,text="Clinic administration",style="Ghost.TButton",command=self._open_clinic_admin).pack(side="left")
        try:
            clinics=list_clinics(self.current_user, include_inactive=True)
            tk.Label(f,text=f"{len(clinics)} clinic(s) configured",bg="white",fg=COLORS["muted"],font=("Segoe UI",9)).pack(anchor="w",padx=18,pady=(0,14))
        except Exception:
            pass

    def _open_clinic_admin(self):
        w=tk.Toplevel(self); w.title("Clinic administration"); w.geometry("760x560"); w.configure(bg=COLORS["bg"])
        c=tk.Frame(w,bg=COLORS["bg"],padx=20,pady=18); c.pack(fill="both",expand=True)
        tk.Label(c,text="Clinic administration",bg=COLORS["bg"],fg=COLORS["ink"],font=("Segoe UI",20,"bold")).pack(anchor="w")
        tk.Label(c,text="Create and deactivate clinics inside the current organization. Changes are audited.",bg=COLORS["bg"],fg=COLORS["muted"],font=("Segoe UI",9)).pack(anchor="w",pady=(2,14))
        form=tk.Frame(c,bg="white",padx=14,pady=14); form.pack(fill="x")
        vars={k:tk.StringVar() for k in ("name","code","address")}
        for i,(k,label) in enumerate((("name","Clinic name"),("code","Code"),("address","Address"))):
            tk.Label(form,text=label,bg="white",fg=COLORS["ink"]).grid(row=i,column=0,sticky="w",padx=(0,10),pady=5); ttk.Entry(form,textvariable=vars[k],width=55).grid(row=i,column=1,sticky="ew",pady=5)
        form.columnconfigure(1,weight=1)
        tree=ttk.Treeview(c,columns=("code","name","status"),show="headings",height=12); tree.heading("code",text="Code"); tree.heading("name",text="Clinic"); tree.heading("status",text="Status"); tree.pack(fill="both",expand=True,pady=14)
        def refresh():
            for x in tree.get_children(): tree.delete(x)
            for cl in list_clinics(self.current_user, include_inactive=True): tree.insert("","end",iid=str(cl["clinic_id"]),values=(cl["code"],cl["name"],"Active" if cl["active"] else "Inactive"))
        def add():
            try: create_clinic(self.current_user,vars["name"].get(),vars["code"].get(),vars["address"].get()); [v.set("") for v in vars.values()]; refresh(); self._status("Clinic created.","success")
            except Exception as exc: messagebox.showerror("Clinic",str(exc),parent=w)
        def toggle():
            sel=tree.selection()
            if not sel: messagebox.showinfo("Clinic","Select a clinic first.",parent=w); return
            row=tree.item(sel[0],"values"); active=row[2] != "Active"
            try: set_clinic_status(self.current_user,sel[0],active); refresh()
            except Exception as exc: messagebox.showerror("Clinic",str(exc),parent=w)
        b=tk.Frame(c,bg=COLORS["bg"]); b.pack(fill="x"); ttk.Button(b,text="Create clinic",style="Accent.TButton",command=add).pack(side="left",padx=(0,8)); ttk.Button(b,text="Activate / deactivate selected",style="Ghost.TButton",command=toggle).pack(side="left")
        refresh()
