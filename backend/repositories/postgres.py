...
from shared.domain import AuthorizationError, Actor
...
class PostgreSQLSessionRepository:
    ...
    def create_session(self, user_id, organization_id, ttl_seconds=3600):
        ...
        with self._connect() as c: 
            c.execute(
                "INSERT INTO api_sessions(organization_id,user_id,token_hash,created_at,expires_at) VALUES(%s,%s,%s,%s,%s)",
                (organization_id, user_id, self._hash(token), now, exp)
            )
        return token
    ...
    def get_session_user(self, token):
        ...
        with self._connect() as c:
            row = c.execute(
                """SELECT u.user_id,u.username,u.email,u.role,u.patient_id,u.full_name,u.active,u.organization_id,u.created_at,s.session_id,s.expires_at 
                   FROM api_sessions s JOIN users u ON u.user_id=s.user_id 
                   WHERE s.token_hash=%s AND s.revoked_at IS NULL AND u.active=TRUE""",
                (self._hash(token),)
            ).fetchone()
            if not row: 
                return None
            if row['expires_at'] <= datetime.now(timezone.utc):
                c.execute("UPDATE api_sessions SET revoked_at=%s WHERE session_id=%s", (datetime.now(timezone.utc), row['session_id']))
                return None
    ...
    def revoke_session(self, token):
        if not token: 
            return
        with self._connect() as c: 
            c.execute("UPDATE api_sessions SET revoked_at=%s WHERE token_hash=%s AND revoked_at IS NULL", (datetime.now(timezone.utc), self._hash(token)))
...
class PostgreSQLRepository:
    ...
    def create_public_user_account(self, username, password, full_name, email=None):
        ...
        with self._connect() as c:
            if c.execute("SELECT 1 FROM users WHERE lower(username)=lower(%s)", (username,)).fetchone():
                raise ValueError("Username is already in use.")
            if email and c.execute("SELECT 1 FROM users WHERE lower(email)=lower(%s)", (email,)).fetchone():
                raise ValueError("Email is already associated with an account.")
            row = c.execute("""INSERT INTO users(organization_id,username,email,password_hash,role,full_name,active)
                               SELECT organization_id,%s,%s,%s,'user',%s,TRUE FROM organizations ORDER BY created_at ASC LIMIT 1
                               RETURNING user_id,username,email,role,patient_id,full_name,active,organization_id,created_at""",
                            (username, email, hash_password(password), full_name)).fetchone()
            if not row:
                raise ValueError("No RecordGuard organization is available for public registration.")
            _pg_audit(c, Actor.from_mapping(dict(row)), "USER_REGISTERED", "identity", "user", row["user_id"], reason_context="Public self-registration")
            return dict(row)

    def create_new_user(self, actor, username, password, role, full_name, patient_id=None, email=None):
        role = str(role or "").strip().lower()
        actor_role = _pg_role(actor)
        if actor_role not in {"owner", "admin"}:
            raise PermissionError("Only Owner or Admin can create user accounts.")
        if actor_role == "admin" and role not in {"doctor", "staff", "patient"}:
            raise PermissionError("Admins can create Doctor, Staff or Patient accounts only.")
        if actor_role == "owner" and role not in {"admin", "doctor", "staff", "patient"}:
            raise ValueError("Invalid user role.")
        username, full_name = str(username or "").strip(), str(full_name or "").strip()
        email = str(email or "").strip().lower() or None
        if len(password or "") < 8: 
            raise ValueError("Password must be at least 8 characters long.")
        with self._connect() as c:
            ...
            if patient_id:
                if role != "patient": 
                    raise PermissionError("Only Patient accounts can be linked to a Patient ID.")
                ...
                if not p: 
                    raise ValueError("The Patient ID is invalid or belongs to another organization.")
                if c.execute("SELECT 1 FROM users WHERE organization_id=%s AND patient_id=%s AND active=TRUE", (actor.organization_id, p["patient_id"])).fetchone(): 
                    raise ValueError("Patient profile is already linked to another active account.")
                patient_uuid = p["patient_id"]
            row = c.execute("""INSERT INTO users(organization_id,username,email,password_hash,role,patient_id,full_name,active)
                               VALUES(%s,%s,%s,%s,%s,%s,%s,TRUE)
                               RETURNING user_id,username,email,role,patient_id,full_name,active,organization_id,created_at""",
                            (actor.organization_id, username, email, hash_password(password), role, patient_uuid, full_name)).fetchone()
            _pg_audit(c, actor, "USER_CREATED", "identity", "user", row["user_id"], reason_context=f"Created {role} account")
            return dict(row)

    def list_users(self, actor):
        _pg_require_role(actor, {"owner", "admin"}, "Only Owner or Admin can view users.")
        with self._connect() as c:
            rows = c.execute("SELECT user_id,username,email,full_name,role,patient_id,active,organization_id,created_at FROM users WHERE organization_id=%s ORDER BY username", (actor.organization_id,)).fetchall()
            return [dict(r) for r in rows]

    def reset_user_password(self, actor, target_user_id, new_password):
        _pg_require_role(actor, {"owner", "admin"}, "Only Owner or Admin can reset another account's password.")
        if len(str(new_password or "")) < 8: 
            raise ValueError("Password must be at least 8 characters long.")
        rid = _pg_uuid(target_user_id, "target_user_id")
        with self._connect() as c:
            target = c.execute("SELECT user_id,username,role,active FROM users WHERE organization_id=%s AND user_id=%s", (actor.organization_id, rid)).fetchone()
            if not target or not target["active"]: 
                raise ValueError("User account not found or inactive.")
            if _pg_role(actor) == "admin" and target["role"] in {"owner", "admin"}: 
                raise PermissionError("Admins cannot reset Owner or Admin passwords.")
            if _pg_role(actor) == "owner" and target["role"] == "owner" and target["user_id"] == actor.user_id: 
                raise ValueError("Use the account password-change workflow for your own Owner account.")
            c.execute("UPDATE users SET password_hash=%s WHERE organization_id=%s AND user_id=%s", (hash_password(new_password), actor.organization_id, rid))
            _pg_audit(c, actor, "USER_PASSWORD_RESET", "identity", "user", rid, reason_context=f"Password reset for {target['username']}")
            return True
    ...
    def create_patient(self, actor, name, age, sex, date_of_birth=None, permanent_address=None,
        ...
        if _pg_role(actor) not in {"owner", "admin", "doctor", "staff"}:
...
def _core15_create_record_shares_batch(self, actor, patient_id, items, shared_with, expires_at=None):
    ...
    with self._connect() as c:
        ...
        for item in items or []:
            ...
            if typ and rid: 
                _pg_share_resource(c, actor, p, typ, rid); clean.append((str(typ).lower(), str(rid)))
...
def _core15_revoke_record_share(self, actor, share_id):
    with self._connect() as c:
        ...
        role = _pg_role(actor)
        if row["shared_by"] != actor.user_id and role != "owner":
            raise PermissionError("Only the creator or Owner can revoke this share.")
...
def _validate_attachment_content(source_path):
    ...
    signatures = {
        ...
        ".doc": head.startswith(b"\xd0\xcf\11\xe0\xa1\xb1\x1a\xe1"),
        ".docx": False,
...
# Bind Core15 methods to the repository class.
...
def _core19_create_correction_request(self, actor, patient_id, resource_type, resource_id, requested_change, reason=None):
    """Create a provenance-preserving request; the source record does not silently mutate.

    Corrections are governed requests and are not direct edits to the original clinical record.
    """
...
def _core34_require_org_admin(self, actor):
    if _pg_role(actor) not in {"owner", "admin"}:
        raise PermissionError("Only an Owner or Admin can manage organization clinics.")
    return _pg_uuid(actor.organization_id, "organization_id")

def _core34_list_clinics(self, actor, include_inactive=False):
    org = self._core34_require_org_admin(actor)
    with self._connect() as c:
        q = "SELECT clinic_id,name,code,address,active,created_at FROM clinics WHERE organization_id=%s"
        if not include_inactive: q += " AND active=TRUE"
        q += " ORDER BY name"
        return [dict(r) for r in c.execute(q, (org,)).fetchall()]

def _core34_create_clinic(self, actor, name, code, address=None):
    org = self._core34_require_org_admin(actor); name = str(name or '').strip(); code = str(code or '').strip().upper()
    if not name or not code: raise ValueError("Clinic name and code are required.")
    if len(code) > 40 or not __import__('re').fullmatch(r"[A-Z0-9][A-Z0-9_-]*", code): raise ValueError("Clinic code must use letters, numbers, hyphens or underscores.")
    import uuid
    cid = uuid.uuid4()
    with self._connect() as c:
        try: c.execute("INSERT INTO clinics(clinic_id,organization_id,name,code,address,created_by) VALUES(%s,%s,%s,%s,%s,%s)", (cid, org, name, code, str(address or '').strip() or None, _pg_uuid(actor.user_id, 'user_id')))
        except UniqueViolation as exc: raise ValueError("A clinic with that code already exists in this organization.") from exc
        _pg_audit(c, actor, "CLINIC_CREATED", "organization", "clinic", cid, None, reason_context=code)
    return str(cid)

def _core34_set_clinic_status(self, actor, clinic_id, active):
    org = self._core34_require_org_admin(actor); cid = _pg_uuid(clinic_id, 'clinic_id')
    with self._connect() as c:
        if not c.execute("SELECT 1 FROM clinics WHERE organization_id=%s AND clinic_id=%s", (org, cid)).fetchone(): raise ValueError("Clinic not found in this organization.")
        c.execute("UPDATE clinics SET active=%s WHERE organization_id=%s AND clinic_id=%s", (bool(active), org, cid))
        _pg_audit(c, actor, "CLINIC_ACTIVATED" if active else "CLINIC_DEACTIVATED", "organization", "clinic", cid, None)
    return True

PostgreSQLRepository.list_clinics = _core34_list_clinics
PostgreSQLRepository.create_clinic = _core34_create_clinic
PostgreSQLRepository.set_clinic_status = _core34_set_clinic_status

def _core34_assign_user_to_clinic(self, actor, target_user_id, clinic_id, role_scope="member"):
    org = self._core34_require_org_admin(actor)
    uid = _pg_uuid(target_user_id, "user_id")
    cid = _pg_uuid(clinic_id, "clinic_id")
    scope = str(role_scope or "member").strip().lower()
    if scope not in {"member", "lead"}:
        raise ValueError("Clinic role scope must be member or lead.")
    with self._connect() as c:
        if not c.execute("SELECT 1 FROM users WHERE organization_id=%s AND user_id=%s AND active=TRUE", (org, uid)).fetchone():
            raise ValueError("User not found in this organization.")
        if not c.execute("SELECT 1 FROM clinics WHERE organization_id=%s AND clinic_id=%s AND active=TRUE", (org, cid)).fetchone():
            raise ValueError("Clinic not found or inactive in this organization.")
        c.execute("""INSERT INTO user_clinics(organization_id,user_id,clinic_id,role_scope)
                     VALUES(%s,%s,%s,%s)
                     ON CONFLICT (organization_id,user_id,clinic_id)
                     DO UPDATE SET role_scope=EXCLUDED.role_scope""", (org, uid, cid, scope))
        _pg_audit(c, actor, "CLINIC_USER_ASSIGNED", "organization", "clinic", cid, None, reason_context=f"user={uid};scope={scope}")
    return True

def _core34_list_user_clinics(self, actor, target_user_id=None):
    org = self._core34_require_org_admin(actor)
    uid = _pg_uuid(target_user_id or actor.user_id, "user_id")
    with self._connect() as c:
        if not c.execute("SELECT 1 FROM users WHERE organization_id=%s AND user_id=%s", (org, uid)).fetchone():
            raise ValueError("User not found in this organization.")
        rows = c.execute("""SELECT uc.clinic_id,c.name,c.code,c.active,uc.role_scope,uc.created_at
                           FROM user_clinics uc JOIN clinics c ON c.clinic_id=uc.clinic_id
                           WHERE uc.organization_id=%s AND uc.user_id=%s ORDER BY c.name""", (org, uid)).fetchall()
        return [dict(r) for r in rows]

def _core34_assign_patient_to_clinic(self, actor, patient_id, clinic_id, primary=False):
    org = self._core34_require_org_admin(actor)
    cid = _pg_uuid(clinic_id, "clinic_id")
    with self._connect() as c:
        pid = _pg_patient_uuid(c, actor, patient_id)
        if not c.execute("SELECT 1 FROM clinics WHERE organization_id=%s AND clinic_id=%s AND active=TRUE", (org, cid)).fetchone():
            raise ValueError("Clinic not found or inactive in this organization.")
        if primary:
            c.execute("UPDATE patient_clinics SET is_primary=FALSE WHERE organization_id=%s AND patient_id=%s", (org, pid))
        c.execute("""INSERT INTO patient_clinics(organization_id,patient_id,clinic_id,is_primary)
                     VALUES(%s,%s,%s,%s)
                     ON CONFLICT (organization_id,patient_id,clinic_id)
                     DO UPDATE SET is_primary=EXCLUDED.is_primary""", (org, pid, cid, bool(primary)))
        _pg_audit(c, actor, "CLINIC_PATIENT_ASSIGNED", "organization", "clinic", cid, pid, reason_context="primary" if primary else None)
    return True

def _core34_list_patient_clinics(self, actor, patient_id):
    """List clinics assigned to a patient, enforcing read permissions for clinicians and patients."""
    with self._connect() as c:
        pid = _pg_patient_uuid(c, actor, patient_id)
        _pg_require_patient_read(c, actor, pid)
        org = _pg_uuid(actor.organization_id, "organization_id")
        rows = c.execute("""SELECT pc.clinic_id,c.name,c.code,c.active,pc.is_primary,pc.created_at
                           FROM patient_clinics pc JOIN clinics c ON c.clinic_id=pc.clinic_id
                           WHERE pc.organization_id=%s AND pc.patient_id=%s
                           ORDER BY pc.is_primary DESC,c.name""", (org, pid)).fetchall()
        return [dict(r) for r in rows]

PostgreSQLRepository.assign_user_to_clinic = _core34_assign_user_to_clinic
PostgreSQLRepository.list_user_clinics = _core34_list_user_clinics
PostgreSQLRepository.assign_patient_to_clinic = _core34_assign_patient_to_clinic
PostgreSQLRepository.list_patient_clinics = _core34_list_patient_clinics